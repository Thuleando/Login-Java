...
CREATE TABLE LockEvents
  (
    ...
    --  'L' = Locked
    --  'U' = Unlocked
    EventType       CHAR (1) DEFAULT 'L' NOT NULL ,
...
Type_ID         Accounttypes.ID%TYPE;

ACCOUNT_EXISTS  Exception;

BEGIN

    SELECT COUNT(Accounts.UserName) INTO ValidUN FROM Accounts 
  ...
  SELECT ID INTO Type_ID FROM AccountTypes WHERE TypeName = 'USER';
  INSERT INTO Accounts (Username, PWord, Accounttypes_Id) VALUES(USERNAME_IN, PASSWORD_IN, Type_ID);
  COMMIT;
  SELECT Accounts.ID INTO AccID FROM Accounts 
               WHERE UserName = USERNAME_IN;
  INSERT INTO LockEvents (Accounts_ID, Eventtype) VALUES(AccID, 'U');
  RESULT := 1;
...
create or replace PROCEDURE Login_SP( UNameEntry    IN Accounts.Username%TYPE,
                                      ...
                                      Account_Type  OUT AccountTypes.TypeName%TYPE)
...
BEGIN
  ...
  IF PWordEntry != PWordVar OR PWordEntry IS NULL THEN
    ...
    SELECT COUNT(Whenattempted) INTO FailedCountVar FROM LoginAttempts
  ...
  ELSE
    ...
    Results := 1;
    SELECT TypeName INTO Account_Type FROM AccountTypes WHERE ID = (
                  SELECT AccountTypes_ID FROM Accounts WHERE Username = UNameEntry);
...
CREATE OR REPLACE PROCEDURE Logout_SP 
(
  USERNAME_IN IN VARCHAR2 
, RESULT      OUT NUMBER 
) 
...
Active_Login INTEGER;
Active_ID    Accounts.ID%TYPE;

BEGIN

SELECT ID INTO Active_ID FROM Accounts WHERE Username = Username_In;

SELECT COUNT(Sessions.Accounts_Id) INTO Active_Login FROM Sessions
  WHERE WhenTerminated IS NULL 
  AND Accounts_Id = Active_ID;
                        
IF Active_Login = 0 THEN
  Result := 2;
ELSE
  Result := 1;
  UPDATE Sessions SET WhenTerminated = LOCALTIMESTAMP 
  WHERE Accounts_ID = Active_ID
  AND WhenTerminated IS NULL;
...
IS
  ...
  ACCOUNTID_NOT_FOUND     EXCEPTION;
  ACCOUNT_NOT_LOCKED      EXCEPTION;
  REASONID_NOT_FOUND      EXCEPTION;

  IsLocked                Accounts.LockStatus%TYPE := 'N';
  ValidAccount            NUMERIC(2) NOT NULL := 0;
  ValidReason             NUMERIC(2) NOT NULL := 0;

BEGIN

  SELECT COUNT(Accounts.ID) INTO ValidAccount FROM Accounts
  WHERE Accounts.ID = ACCOUNTID_IN;
  IF (ValidAccount) = 0 THEN
    RAISE ACCOUNTID_NOT_FOUND;
  END IF;
  
  SELECT Accounts.LockStatus INTO IsLocked FROM Accounts
  WHERE Accounts.ID = ACCOUNTID_IN;
  IF IsLocked = 'N' THEN
    RAISE ACCOUNT_NOT_LOCKED;
  END IF;

  SELECT COUNT(EventReasons.ID) INTO ValidReason FROM EventReasons
  WHERE EventReasons.ID = REASONID_IN
  AND EventReasons.ReasonType = 'U';
  IF (ValidReason) = 0 THEN
    RAISE REASONID_NOT_FOUND;
  END IF;
  ...
  DBMS_OUTPUT.PUT_LINE('Account #' || ACCOUNTID_IN || ' unlocked.');
  RESULT := 1;

EXCEPTION
  WHEN ACCOUNTID_NOT_FOUND THEN
    DBMS_OUTPUT.PUT_LINE('Account #' || ACCOUNTID_IN || ' does not exist.');
    RESULT := 2;
  WHEN ACCOUNT_NOT_LOCKED THEN
    DBMS_OUTPUT.PUT_LINE('Account #' || ACCOUNTID_IN || ' is already unlocked.');
    RESULT := 3;
  WHEN REASONID_NOT_FOUND THEN
    DBMS_OUTPUT.PUT_LINE('EventReason #' || REASONID_IN || ' is not a valid unlock reason.');
    RESULT := 4;
  WHEN OTHERS THEN
